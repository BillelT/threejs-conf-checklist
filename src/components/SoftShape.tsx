...
type Props = {
  ...
  strength?: number // amount pushed toward pointer per frame
  spring?: number // rest return stiffness (0..1 per step)
  damping?: number // velocity multiplier per step
...
function buildPerimeter(
  ...
  type Seg =
    ...
    | { kind: 'arc'; cx: number; cy: number; a0: number; a1: number; len: number }
  const segs: Seg[] = [
...
export function SoftShape({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (Math.abs(prev.w - w) < 0.5 && Math.abs(prev.h - h) < 0.5 && pointsRef.current.length) {
        return
      }
      sizeRef.current = { w, h }
    ...
    const step = () => {
      ...
      const active =
        ...
        p.screenY < rect.bottom + influenceRadius
  ...
  }, [radius, perimeterPoints, influenceRadius, strength, spring, damping, contentParallax, push])
